...
const StartApp = async query => {
    ...
    const [stillToProcess, NumTicketPurchasesRows, totalTicketsSold, txnSeenSoFar] = await getTransactionNumbers(page)
    ...
    const totalSalesForEvent = parseFloat(ticketPurchases.reduce(function (acc, curr) {
    ...
    const jsonContent = JSON.stringify({ NumTixSold, totalSalesForEvent, eventData, ticketPurchases, ticketTypes, AdjustmentsListing, CommissionInvoices });
...
const myBatchOfInvoices = async enrichedTicketsDataArray => {
    ...
    const extractClientOwners = enrichedTicketsDataArray.map(a => a.clientOwnerAtPurchaseDate);

    const uniqueListClientOwners = extractClientOwners.filter(onlyUnique);
    ...
    for (var agent of uniqueListClientOwners) {
        ...
        const extractEventOwners = oneGroupBookings.map(a => a.eventOwner);
        const extractTicketCategories = oneGroupBookings.map(a => a.ticketoption);
        const uniqueListEventOwners = extractEventOwners.filter(onlyUnique);
        const extractEventNames = oneGroupBookings.map(a => a.eventName);
        const extractEventRefs = oneGroupBookings.map(a => a.eventRef);
        const extractEventTypes = oneGroupBookings.map(a => a.eventType);
        const extractEventCommences = oneGroupBookings.map(a => a.eventCommences);
        const uniqueListEventRefs = extractEventRefs.filter(onlyUnique);
        const uniqueListEventTypes = extractEventTypes.filter(onlyUnique);
        const uniqueListEventNames = extractEventNames.filter(onlyUnique);
        const uniqueListTicketOptions = extractTicketCategories.filter(onlyUnique);
        const uniqueListEventCommences = extractEventCommences.filter(onlyUnique);
        ...
        if (invoiceTo != invoiceFrom){
            ...
            debugger;
            const ticketamounts = oneGroupBookings.map(a => a.totaldueCalc);
            const discountamounts = oneGroupBookings.map(a => Number(a.discountTotal));
            ...
            for (var tickettype of uniqueListTicketOptions) {
                const newArray = oneGroupBookings.filter(function (el) {
                    return el.clientOwnerAtPurchaseDate == agent &&
                         el.ticketoption == tickettype
                });
                // console.log(newArray,'NEW ARRAY')
                const arrayOfTicketQuantities = newArray.map(a => a.ticketquantity);
                const arrayOfTicketPrices = newArray.map(a => a.ticketprice);
                const arrayOfDiscountAmounts = newArray.map(a => Number(a.discountTotal));
                let totalcategorydiscount = 0;
                let ticketcategorytotaldue = 0;
                for (let i=0; i< arrayOfTicketQuantities.length; i++) {
                ...
                const ticketsubtotalamounts = newArray.map(a => Number(a.totaldueCalc));
...
const goToEventsListing = async page => {
    ...
    const eventsWindow = await page.frames()[5]
...
const switchToTraitsTab = async page => {
    ...
    const element = await eventsDetailWindow.$('#ctl00_Content_ucxEvents_Edit_tabSet_tab6_ucxEventTraitsEditor_rptTraitTypes_ctl00_rptTraits_ctl01_divTraitButton')
    const value = await eventsDetailWindow.evaluate(el => el.getAttribute('class'), element)
    console.log(value, 'IM THE TRAITS CLASS VALUE')
    const result = value.includes('TraitActive');
    console.log(result, 'im the result')
}

const clickOnTicketPurchases = async page => {
    const eventsTicketPurchaseTab = await page.frames()[7]
...
async function getTransactionNumbers(page) {
    ...
    const eventsTicketPurchaseTab = await page.frames()[7]
    const NumTransactionsDataLabel = await eventsTicketPurchaseTab.$eval('.EventBookingsTicketPurchases > div:nth-child(1) > div:nth-child(4)', el => el.innerText)
    const NumTicketsSoldInTotal = NumTransactionsDataLabel.split(/(\s+)/)
    const txnSeenSoFar = NumTicketsSoldInTotal[6]
    const totalTicketsSold = NumTicketsSoldInTotal[10]
    const tableSelector = '.EventBookingsTicketPurchases > div:nth-child(1) > table:nth-child(3) > tbody:nth-child(2)'
    const NumTicketPurchasesRows = await eventsTicketPurchaseTab.$$eval(`${ tableSelector } > tr`, el => el.length)
    const stillToProcess = (totalTicketsSold - txnSeenSoFar)
...
const getPageNumbers = async page => {
    const eventsTicketPurchaseTab = await page.frames()[7]
    const PagingLabel = await eventsTicketPurchaseTab.$eval('.EventBookingsTicketTypes > div:nth-child(1) > div:nth-child(4)', el => el.innerText)
    const NumTicketsSoldInTotal = PagingLabel.split(/(\s+)/)
    const totalRowsOnTable = NumTicketsSoldInTotal[10]
...
async function getTransactionsDetails(page, NumTicketPurchasesRows, bookings) {
    try {
        ...
        const tableSelector = '.EventBookingsTicketPurchases > div:nth-child(1) > table:nth-child(3) > tbody:nth-child(2)'
        debugger;
        const i = NumTicketPurchasesRows
...
async function getBookings(page) {
    ...
    for (let j = 0; j < lastPageOfLoop; j++) {
        const numTableRows = await eventsDetailWindow.$$eval('.table > tbody > tr', elements => elements.length)
        ...
        for (let i = 0; i <= numTableRows - 1; i++) {
            ...
            i = n(i)
            const bookingRef = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColBookingReference`, td => td.innerText)
            const bookedOn = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColBookedOn`, td => td.innerText)
            const clientFirstName = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColFirstName`, td => td.innerText)
            const clientLastName = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColLastName`, td => td.innerText)
            const clientOwnerAtPurchaseDate = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColClientOwningSubAccountGID`, td => td.innerText)
            const eventName = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColEventName`, td => td.innerText)
            const eventCommences = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColCommences`, td => td.innerText)
            const eventOwner = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColEventOwningSubAccountGID`, td => td.innerText)
            const ticketQuantity = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColTicketQty`, td => td.innerText)
            const bookingStatus = await eventsDetailWindow.$eval(`#ctl00_Content_ucxEvents_Edit_tabSet_tab1_ucxEventsBookings_List_rptList_ctl${ i }_trRow > td.ColBookingStatusGID`, td => td.innerText)
...
const calcCommissions = (agentCode, bookings) => {
    ...
    const filteredTrans= ticketPurchases.filter(tix => tix.agentCode === agentCode);
    // console.log(filteredTrans, 'transactions only for agent code:',agentCode)
    const totalSales = parseFloat(filteredTrans.reduce(function (acc, curr) {
        return acc + curr.ticketquantity * curr.ticketprice;
    }, 0)).toFixed(2)
    const totalCommissionPayable = parseFloat(filteredTrans.reduce(function (acc, curr) {
        return acc + curr.commissionpayable
    }, 0)).toFixed(2)
    const cashExpectedToBeReceived = parseFloat((totalSales - totalCommissionPayable)).toFixed(2)
    const totalTicketsSold = parseInt(filteredTrans.reduce(function (acc, curr) {
...
async function getTicketTypeDetails(page) {
    ...
    const eventsTicketPurchaseTab = await page.frames()[7]
    const totalRowsOnTable= await getPageNumbers(page)
    console.log(totalRowsOnTable)
    const tableSelector = '.EventBookingsTicketTypes > div:nth-child(1) > table:nth-child(3) > tbody:nth-child(2)'
...
const getEventSetupData = async page => {
    ...
    const getEventType = await eventsDetailWindow.evaluate(()=> {
        const eventTypeSelected = document.querySelectorAll('#ctl00_Content_ucxEvents_Edit_tabSet_tab0_ddlEventTypeID')[0].selectedOptions[0].innerText
        return eventTypeSelected;
    });

    const getCommencesTime = await eventsDetailWindow.evaluate(() => {
        const el = document.querySelector('#ctl00_Content_ucxEvents_Edit_tabSet_tab0_txtCommences_Time')
    ...
    const getFinishesTime = await eventsDetailWindow.evaluate(() => {
        ...
        const el = document.querySelector('#ctl00_Content_ucxEvents_Edit_tabSet_tab0_txtFinishes_Time')
        return el ? el.getAttribute('value') : '00:00'
    })
    const getEventStatus = await eventsDetailWindow.evaluate(()=> {
        const eventStatusSelected = document.querySelectorAll('#ctl00_Content_ucxEvents_Edit_tabSet_tab0_ddlEventStatusID')[0].selectedOptions[0].innerText
        return eventStatusSelected;
    });
    const getEventOwner = await eventsDetailWindow.evaluate(()=> {
        const eventOwnerSelected = document.querySelectorAll('#ctl00_Content_ucxEvents_Edit_tabSet_tab0_ddlOwningSubAccount')[0].selectedOptions[0].innerText
    ...
    const getEventCancellationPolicy = await eventsDetailWindow.evaluate(()=> {
        const eventCancellationPolicySelected= document.querySelectorAll(' #ctl00_Content_ucxEvents_Edit_tabSet_tab0_ddlCancellationPolicy')[0].selectedOptions[0].innerText
...
const loopAllTicketPurchasePages = async (stillToProcess, NumTicketPurchasesRows, totalTicketsSold, txnSeenSoFar, page, bookings) => {
    ...
    const [stillToProcess1, NumTicketPurchasesRows1, totalTicketsSold1, txnSeenSoFar1]=await getTransactionNumbers(page)
...
const enrichTheTicketData = async (AdjustmentsListing, ticketPurchases, eventData) => {
    ...
    const jsonContent3 = JSON.stringify(enrichedTicketsArray);
...
(async()=>{
    ...
    const events = JSON.parse(eventsprocessedjson);
    const difference = myEvents.filter(x => !events.includes(x));
    console.log(difference)
    const arrayLength = difference.length;
    console.log(myEvents)
    for (let i = 0; i < arrayLength; i++) {
        console.log(difference[i]);
        const query = difference[i](async () => {
